...
一个自动化清洗、分类的 DNS 分流规则集，专为多出口 network 环境与企业级 DNS 转发引擎（如 Mosdns）量身定制。
...
* **纯净无状态**：规则仅包含纯文本域名或 IP 列表（一行一个），不附带任何代理节点、解密密钥或具体的解析逻辑。
* **全球加速**：全线托管于 jsDelivr CDN，支持多分支网关设备（如 RouterOS、Linux）高速、稳定地拉取。
* **业务无感知**：配合策略路由或 DNS 转发矩阵，可有效优化跨国办公延迟并规避潜在的解析污染。
